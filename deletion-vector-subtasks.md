...
2. 提供 `open()`、`close()` 方法
3. 提供 `createCheckpoint()` 方法（用于 §11 恢复）
4. 支持 WriteBatch 原子写入（参考 `RocksDBWriteBatchWrapper`）
5. DvRocksDB 文件路径与 KvTablet RocksDB 路径分离
...
**目标**：创建 DvManager 作为所有 DV 操作的统一入口，内置 dvLock 并发控制，管理 snapshotBitmap 和 knownFiles 生命周期。
...
5. **DvManager 持有以下状态**（后续 subtask 逐步填充实现）：
   ...
   - `Set<String> knownFiles` — 已知文件集合（内存），由 §7.3 步骤 2 添加，§8.2 Step 3 移除 oldFiles。用于 §8.1 区分 Fluss 自己写的文件 vs 外部 compaction 文件。
...
**目标**：改造 RowDelta commit 逻辑，增加 `validateFromSnapshot` 和 `validateDataFilesExist`。
...
1. 改造 RowDelta commit（DV 模式下）：
   ...
   rowDelta.validateDataFilesExist(lakeDvReferencedDataFiles);
   ...
   ```
2. `lakeDvReferencedDataFiles`：LakeDv 快照过滤后引用的已有 data file（不含本次新增 data file）
3. `baseSnapshotId`：读取 table state 时的 snapshot id
4. 冲突处理：`ValidationException` → abort，清理已生成的 Puffin DV 和 data file
5. WriteResult 扩展：包含 `positionReport`、`locallyDeletedRowIds`、`materializedDvFiles`、`actualSnapshotId`

**验证**：集成测试模拟并发 compaction 场景，验证 ValidationException 处理正确。
...
4. **步骤 2**：将 file_path 加入 `knownFiles`
5. **步骤 3**：对每个 `(RowId, row_position)`，在 FileDict 中查找或创建 file_id
6. **步骤 4**：按 RowId 范围统一处理：
   ...
   - RowId ∉ splitOffsetRange（重写行）→ 查 RowPosIndex/pendingRowPos：
     ...
     - 都找不到 → 标记 LakeDv
7. **步骤 5**：遍历 `locallyDeletedRowIds`，从 PendingDeletes 移除
8. **步骤 6**：WriteBatch 原子提交步骤 2-5 的所有修改
9. **步骤 7**：释放 `dvLock.writeLock()`
10. **步骤 8**（在 dvLock 外执行）：用 `materializedDvFiles` 过滤 DvManager 内存中的 `snapshotBitmap`，仅保留已物化的文件。此步骤不需要 dvLock 因为 snapshotBitmap 只在 position report 处理链路中被修改，由步骤 0 的过期检查保证不会有并发的 split 生成覆盖
11. **步骤 9**：发送 DV 完成通知（**必须在步骤 8 成功之后**，若步骤 8 失败则不发送通知，记录错误日志，下轮 tiering 重试）
...
4. **新增 RPC 定义**（FlussApi.proto）：
   ...
   message NotifyReadableSwitchRequest {
     ...
     // oldFiles 和 newFiles 通过 snapshot diff 由 TabletServer 本地计算，
     // 或由 CoordinatorServer 计算后下发
   }
...
2. **在 dvLock.writeLock() 下原子执行**：
   ...
   - b. 清空 pendingRowPos
   - c. 对 oldFiles：删除 LakeDv 条目 + 从 knownFiles 移除
...
### Subtask 6-1：DvRocksDB Checkpoint 实现（§11.1）

**设计参考**：§11.1

**依赖**：Subtask 1-1

**目标**：实现 DvRocksDB 的 checkpoint 机制。

**涉及文件（新建）**：
- `fluss-server/src/main/java/org/apache/fluss/server/kv/dv/DvCheckpoint.java`

**实现要点**：
1. Checkpoint 内容：DvRocksDB SST 文件 + 元数据
2. 元数据记录：
   - `restoreSnapshot`：当前 DV-readable snapshot ID
   - `snapshotStartLogOffset`：该 snapshot 对应的 changelog start offset
   - `checkpointLogHw`：checkpoint 时刻的 log_hw
3. 触发时机：每次 readable snapshot 前移（§8.2 Step 3）完成后触发
4. SST 文件上传到远程存储——参考现有 `RocksIncrementalSnapshot`（`fluss-server/src/main/java/org/apache/fluss/server/kv/snapshot/RocksIncrementalSnapshot.java`）和 `KvTabletSnapshotTarget`，可复用增量快照上传逻辑
5. 降级策略：checkpoint 失败记录日志，下次 readable 前移时重试，不影响正常运行

---

### Subtask 6-2：DvRocksDB 恢复流程实现（§11.2）

**设计参考**：§11.2

**依赖**：Subtask 6-1

**目标**：实现 DvRocksDB 从 checkpoint 恢复的完整流程。

**涉及文件**：
- `fluss-server/src/main/java/org/apache/fluss/server/kv/dv/DvRecoverHelper.java`（新建）
- `fluss-server/src/main/java/org/apache/fluss/server/kv/dv/DvManager.java`

**实现要点**：
1. **Step 1**：从远程存储拉取 SST，加载 DvRocksDB
2. **Step 2-3**：从 `checkpointLogHw + 1` 重放 changelog：
   - 对每条 `-U`/`-D`：
     - 查 RowPosIndex → 命中则更新 LakeDv + 删 RowPosIndex；未命中则加 PendingDeletes
     - 比较 `oldRowId` 与 `snapshotStartLogOffset` 决定是否更新 LogDv
3. **Step 4**：处理 checkpoint 之后的新 snapshot（targetSnapshot）：
   - 计算 newFiles
   - 扫描 newFiles（读 `__offset` + `__bucket`），重建位置状态
   - 按 RowId 范围 + PendingDeletes 状态决定写 pendingRowPos 还是标记 LakeDv
   - 上报 ready 给 CoordinatorServer
   - 不在本地执行 readable 切换
   - snapshotBitmap 未填充，跳过差集清理

**验证**：恢复后状态测试 — 验证 RowPosIndex、LakeDv、LogDv 正确。

---

## Phase 7：外部 Compaction 交互

> 依赖 Phase 3 + Phase 4。

### Subtask 7-1：外部 Compaction 检测与文件扫描（§12.1, §12.2）

**设计参考**：§12

**依赖**：Phase 3

**目标**：Tiering Service commit 时检测外部 compaction，扫描新文件并分发 position。

**涉及文件**：
- Tiering Service 相关代码（commit 时新增检测逻辑）
- `fluss-lake/fluss-lake-iceberg/src/main/java/org/apache/fluss/lake/iceberg/tiering/IcebergLakeCommitter.java`
...
1. 检测外部 compaction：
   ...
   ```
2. 扫描 externalNewFiles：读 `__offset` 和 `__bucket` 列
3. **校验系统列**：检查 `__offset` 和 `__bucket` 是否存在且类型正确
   - 缺失/类型不匹配 → 上报 metric `external_compaction_invalid_files`，阻止 DV-readable 前移
4. 按 bucket 分组，与本轮 positionReport 合并上报
5. 上报 externalOldFiles 给 TabletServer

**验证**：测试外部 compaction 文件检测、系统列校验、按 bucket 分组。

---

### Subtask 7-2：Snapshot 过期策略与可观测性（§12.4, §12.5）

**设计参考**：§12.4, §12.5

**依赖**：无强依赖，可与 Phase 7 其他 subtask 并行

**目标**：配置 Iceberg snapshot 过期策略保护 readable snapshot，添加可观测性 metric。

**涉及文件**：
- `fluss-lake/fluss-lake-iceberg/src/main/java/org/apache/fluss/lake/iceberg/IcebergLakeCatalog.java` — 设置 snapshot 保留策略
- Metrics 相关代码

**实现要点**：
1. 建议配置 `history.expire.min-snapshots-to-keep` 足够大
2. 在 table property 中标记当前 readable snapshot id
3. 新增 metric：`external_compaction_files_scanned`
4. 新增 metric：`external_compaction_invalid_files`

---

## Phase 8：LakeDv 物化完整流程

> 这是 §13 描述的完整物化流程，涉及 Phase 3 和 Phase 4 的协调。主要是集成验证。

### Subtask 8-1：LakeDv 物化端到端集成（§13）
...
### Subtask 9-1：端到端测试 — 设计文档 §14 示例场景
...
**目标**：测试 failover 恢复后 DV 状态正确。

**测试场景**：
1. 正常运行 → checkpoint → 写入更多数据 → failover
2. 恢复后验证 RowPosIndex/LakeDv/LogDv 状态
3. 恢复后 union read 正确

### 🔀 Subtask 9-3：外部 Compaction 场景测试

**目标**：测试外部 compaction 后 DV 仍正确工作。

**测试场景**：
1. 正常 tiering 后，外部 Spark 执行 compaction 替换文件
2. 下一轮 tiering 检测到外部 compaction
3. 扫描新文件 → position report → RowPosIndex 更新
4. Union read 结果正确
...
Phase 4 (Snapshot 处理)
  ...
  └── 4-3 初始构建处理
                ┌────────┴────────┐
                ▼                 ▼
Phase 5 (Union Read)     Phase 6 (恢复)        Phase 7 (外部 Compaction)
  ├── 5-1 Server 接口      ├── 6-1 Checkpoint    ├── 7-1 检测与扫描
  ├── 5-2 RPC 🔀           └── 6-2 恢复流程      └── 7-2 Metrics 🔀
  └── 5-3 Client 适配
                ▼                 ▼                       ▼
           Phase 8 (物化集成验证)
                         ▼
...
- Phase 5、Phase 6、Phase 7 在 Phase 4 之后**可并行**
- Phase 9 的 4 个测试 subtask **可并行**
...
### 第五批（Phase 5/6/7 同时开 3 个 sub-agent）
- Agent M → Subtask 5-2（Union Read RPC）→ Subtask 5-1 + 5-3（Server + Client）
- Agent N → Subtask 6-1 + 6-2（恢复流程）
- Agent O → Subtask 7-1 + 7-2（外部 Compaction）

### 第六批
- Agent P → Subtask 8-1（LakeDv 物化端到端集成验证）

### 第七批（同时开 4 个 sub-agent）
- Agent Q → Subtask 9-1（§14 端到端测试）
- Agent R → Subtask 9-2（恢复场景测试）
- Agent S → Subtask 9-3（外部 Compaction 测试）
- Agent T → Subtask 9-4（并发场景测试）

> **总计 20 个 sub-agent，分 7 批执行。第一批最大化并行度（8 个），总体开发效率最高。**
