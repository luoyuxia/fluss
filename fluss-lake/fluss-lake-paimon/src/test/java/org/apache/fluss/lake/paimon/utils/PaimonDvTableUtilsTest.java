...
class PaimonDvTableUtilsTest {
    ...
    void testFindLatestSnapshotExactlyHoldingL0Files() throws Exception {
        ...
        Map<Integer, List<GenericRow>> appendRows =
                Map.of(
                        bucket0, generateRows(0, 3),
                        bucket1, generateRows(0, 3));
        ...
        //   └─────────┴─────────────────────────────┘
        CompactCommitter compactCommitter1_b0 = compactHelper.compactBucket(bucket0);
        compactCommitter1_b0.commit();

        // Prepare compact for bucket1 (not committed yet)
        CompactCommitter compactCommitter1_b1 = compactHelper.compactBucket(bucket1);
        ...
        //   │ bucket1 │ L0: [rows 0-2]              │ ← unchanged
        //   └─────────┴─────────────────────────────┘
        CompactCommitter compactCommitter3_b0 = compactHelper.compactBucket(bucket0);
        compactCommitter3_b0.commit();
