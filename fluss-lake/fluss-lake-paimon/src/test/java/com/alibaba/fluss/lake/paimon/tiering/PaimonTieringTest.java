...
class PaimonTieringTest {
    ...
    private void verifyPrimaryKeyTableRecord(
            CloseableIterator<InternalRow> actualRecords, List<LogRecord> expectRecords)
            throws Exception {
        for (LogRecord expectRecord : expectRecords) {
            InternalRow actualRow = actualRecords.next();
    ...
    private Tuple2<List<LogRecord>, List<LogRecord>> genPrimaryKeyTableRecords(
        ...
        List<LogRecord> writtenLogRecords =
                new ArrayList<>(
                        Arrays.asList(
                                toRecord(rows.get(0), INSERT),
                                toRecord(rows.get(1), UPDATE_BEFORE),
                                toRecord(rows.get(2), UPDATE_AFTER),
                                toRecord(rows.get(3), DELETE)));
        ...
        writtenLogRecords.addAll(
                Arrays.asList(
                        ...
                        toRecord(rows.get(1), UPDATE_BEFORE),
                        toRecord(rows.get(2), UPDATE_AFTER)));
        expectLogRecords.add(toRecord(rows.get(2), UPDATE_AFTER));

        // gen +I, +U
        rows = genKvRow(partition, bucket, 2, 7, 9);
        writtenLogRecords.addAll(
                Arrays.asList(toRecord(rows.get(0), INSERT), toRecord(rows.get(1), UPDATE_AFTER)));
        expectLogRecords.add(toRecord(rows.get(1), UPDATE_AFTER));

        // gen +I
        rows = genKvRow(partition, bucket, 3, 9, 10);
        writtenLogRecords.add(toRecord(rows.get(0), INSERT));
        expectLogRecords.add(toRecord(rows.get(0), INSERT));
    ...
    private GenericRecord toRecord(GenericRow row, ChangeType changeType) {
        return new GenericRecord(row.getInt(0), System.currentTimeMillis(), changeType, row);
    }
