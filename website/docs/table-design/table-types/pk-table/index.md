...
key(must be a subset of primary keys excluding partition keys) for each record. If the bucket key is not specified, the bucket key will be the primary key(excluding partition key).
