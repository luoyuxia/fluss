...
- **Server-side** (C.2, C.3): Historical operations are offloaded from RPC threads to a dedicated `ioExecutor` and admitted through a separate historical request semaphore. Real-time paths never use `ioExecutor`, and normal lookups do not pass through the historical semaphore.
- **Client-side** (C.4): The first version does not introduce `client.lookup.historical-inflight-ratio` and does not split lookup inflight permits. The client mainly reuses retry/backoff for server-side throttle errors.
...
| Server | Historical request semaphore | `netty.server.max-queued-historical-requests` (C.3) | `tryAcquire()` fails → `HISTORICAL_PARTITION_THROTTLED` |
| Client | Write throughput | Bounded by server semaphore (C.4.1) | Server throttle + client exponential backoff (100ms, 2×, max 5s) |
| Client | Lookup inflight permits | No separate historical cap in first version (C.4.2) | Existing lookup inflight permits + server throttle/backoff |
...
#### C.4 Client-side: Historical Throttle Handling

The first version does not add a dedicated historical inflight ratio and does not split `LookupSender` normal/historical permits. The client only handles historical throttle errors returned by the server and reuses the existing retry/backoff path.
...
- Historical write throughput is bounded by `netty.server.max-queued-historical-requests`. Client automatically adapts its sending rate via backoff.
...
##### C.4.2 Lookup path: reuse existing inflight permits
...
The first version keeps using the existing lookup inflight permits and does not add `client.lookup.historical-inflight-ratio`. When `LookupSender` receives a `HISTORICAL_PARTITION_THROTTLED` error from the server, it applies the same exponential backoff strategy as the write path (`ExponentialBackoff(100, 2, 5000, 0.2)`), setting `retryAfterMs` on the lookup query to delay the retry.

Effects:

- Server-side historical request admission prevents historical lake fallback requests from growing without bound.
- When historical admission is exhausted, the server returns throttle errors and the client retries with backoff.
