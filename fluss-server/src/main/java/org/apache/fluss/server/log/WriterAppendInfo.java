...
public class WriterAppendInfo {
     ...
     *             sequence. Here is a detailed example illustrating the scenario described above:
     *             The expiration of a writer is triggered asynchronously by the {@code
     *             PeriodicWriterIdExpirationCheck} thread at intervals defined by {@link
     *             org.apache.fluss.config.ConfigOptions.WRITER_ID_EXPIRATION_CHECK_INTERVAL}, which
     *             can result in slight differences in the actual expiration times of the same
     *             writer on the leader replica and follower replicas. This slight difference leads
     *             to a dreadful corner case. Imagine the following scenario(set {@link
     *             org.apache.fluss.config.ConfigOptions.WRITER_ID_EXPIRATION_CHECK_INTERVAL}:
     *             10min, {@link org.apache.fluss.config.ConfigOptions.WRITER_ID_EXPIRATION_TIME}:
     *             12h):
     *             <pre>
     ...
     *             OutOfOrderSequenceException} will occur during to write.
     *       </ul>
