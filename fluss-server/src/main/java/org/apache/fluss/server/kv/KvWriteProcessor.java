...
/** Shared record merge and WAL processor for normal and historical KV writes. */
@Internal
@NotThreadSafe
public final class KvWriteProcessor implements AutoCloseable {
    ...
    private long processDeletion(
        ...
        byte[] oldValueBytes = getFromState(primaryKey, key, stateAccessor);
    ...
    private byte[] getFromState(
            byte[] primaryKey, KvPreWriteBuffer.Key encodedKey, KvStateAccessor stateAccessor)
            throws Exception {
        return stateAccessor.lookup(primaryKey, encodedKey).value();
    }
