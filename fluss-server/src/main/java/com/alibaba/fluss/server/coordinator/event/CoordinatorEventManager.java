        ...
        AccessContextEvent<MetricsData> accessContextEvent =
                new AccessContextEvent<>(
                        context -> {
                            ...
                            StringBuilder serverStats = new StringBuilder("Tablet server status: ");
                            serverStats
                                    .append("living=")
                                    .append(tabletServerCount)
                                    .append(", maxId=")
                                    .append(maxTabletServerId)
                                    .append(", offline=[");
                            boolean hasOfflineServers = false;
                            for (int i = 1; i <= maxTabletServerId; i++) {
                                if (!liveTabletServers.containsKey(i)) {
                                    if (hasOfflineServers) {
                                        serverStats.append(", ");
                                    }
                                    serverStats.append(i);
