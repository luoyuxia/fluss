...
/** Abstract common operations for merge engine. */
public abstract class RowMergeEngine {

    private static final Logger LOG = LoggerFactory.getLogger(RowMergeEngine.class);
    ...
    public void writeRecord(KvRecord kvRecord) throws Exception {
        ...
        if (kvRecord.getRow() == null) {
            ...
            if (oldValue == null) {
                ...
                deleteOrUpdate(key, oldValue);
            }
        } else {
            // upsert operation
            upsert(key, kvRecord, oldValue);
        }
    }

    private void upsert(KvPreWriteBuffer.Key key, KvRecord kvRecord, byte[] oldValue)
            throws Exception {
        // it's update
        if (oldValue != null) {
