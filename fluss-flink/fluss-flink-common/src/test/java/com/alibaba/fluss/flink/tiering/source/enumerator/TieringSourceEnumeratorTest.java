...
class TieringSourceEnumeratorTest extends TieringTestBase {
    ...
    void testPrimaryKeyTableWithNoSnapshotSplits() throws Throwable {
        ...
        try (MockSplitEnumeratorContext<TieringSplit> context =
            ...
            context.getSplitsAssignmentSequence()
                    .forEach(a -> a.assignment().values().forEach(r -> actualAssignment.addAll(r)));
            ...
            coordinatorGateway.lakeTieringHeartbeat(
                    TieringSourceEnumerator.HeartBeatHelper.finishedTieringTableHeartBeat(
                            tableId, enumerator.getFlussCoordinatorEpoch()));
            ...
            // TODO: Looks like configured table.datalake.freshness in LakeTableTieringManager
    ...
    void testLogTableSplits() throws Throwable {
        ...
        try (MockSplitEnumeratorContext<TieringSplit> context =
            ...
            // TODO: Looks like configured table.datalake.freshness doesn't work in
    ...
    void testPartitionedPrimaryKeyTable() throws Throwable {
        ...
        try (MockSplitEnumeratorContext<TieringSplit> context =
            ...
            for (Map.Entry<Long, String> partitionNameById : partitionNameByIds.entrySet()) {
                for (int tableBucket = 0; tableBucket < DEFAULT_BUCKET_NUM; tableBucket++) {
                    long partitionId = partitionNameById.getKey();
            ...
            // mock finished tiered this round, check second round
            context.getSplitsAssignmentSequence().clear();
            final Map<Long, Map<Integer, Long>> initialBucketOffsets = new HashMap<>();
            for (Map.Entry<Long, String> partitionNameById : partitionNameByIds.entrySet()) {
                Map<Integer, Long> partitionInitialBucketOffsets = new HashMap<>();
                for (int tableBucket = 0; tableBucket < DEFAULT_BUCKET_NUM; tableBucket++) {
                    partitionInitialBucketOffsets.put(tableBucket, EARLIEST_OFFSET);
                }
                initialBucketOffsets.put(partitionNameById.getKey(), partitionInitialBucketOffsets);
                ...
                coordinatorGateway
                        .commitLakeTableSnapshot(
                                genCommitLakeTableSnapshotRequest(
                                        ...
                                        bucketOffsetOfInitialWrite.get(partitionNameById.getKey())))
                        .get();
            }
            ...
            for (Map.Entry<Long, String> partitionNameById : partitionNameByIds.entrySet()) {
                for (int tableBucket = 0; tableBucket < DEFAULT_BUCKET_NUM; tableBucket++) {
                    long partionId = partitionNameById.getKey();
    ...
    void testPartitionedLogTableSplits() throws Throwable {
        ...
        try (MockSplitEnumeratorContext<TieringSplit> context =
            ...
            // mock finished tiered this round, check second round
            context.getSplitsAssignmentSequence().clear();
            final Map<Long, Map<Integer, Long>> bucketOffsetOfEarliest = new HashMap<>();
            ...
            for (Map.Entry<Long, String> partitionNameById : partitionNameByIds.entrySet()) {
                ...
                bucketOffsetOfEarliest.put(partitionId, partitionInitialBucketOffsets);
